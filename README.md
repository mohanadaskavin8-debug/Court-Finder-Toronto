...
- **Accuracy verification (July 2026):** All 4 community centres (Warden Hilltop, Canoe Landing, Antibes, Port Union) confirmed to have an "Outdoor Basketball Court" on their official toronto.ca location pages. Park spot-checks (Christie Pits, Tall Pines, Flemingdon) also confirmed outdoor courts (some locations have multiple physical courts; the dataset models one entry per location). The city's Open Data lists no other community centres with basketball courts. School courts remain inferred; a disclosure note now appears in the detail panel and list view.
